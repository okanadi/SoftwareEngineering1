<script lang="ts">
  ...
  function handleStepCreated(event: CustomEvent) {
    ...
    showModal = false;
    // Optionally refresh the project data or navigate to the updated project
  }
</script>
  ...
  <div class="container mx-auto p-4 md:p-8">
    {#if data.project}
      <div class="flex justify-between items-center mb-8">
        <h1 class="text-4xl font-bold text-white">Project Details</h1>
      ...
      <CreateProjectStepModal projectId={data.project.id} isOpen={showModal} on:stepCreated={handleStepCreated} />
    {:else if data.error}
